...
class MockLLM:
    ...
    def complete(self, messages: Sequence[Message]) -> str:
        ...
        if "P7_REFLEXION_RETRY" in user_text:
            draft_m = re.search(r"Previous draft:\s*(.+?)\nReflection:", user_text, re.S)
            question_m = re.search(r"Question:\s*(.+?)\nPrevious draft:", user_text, re.S)
            ...
            return normalize_compact(draft, prompt=question) or draft
        if "P7_REFLEXION" in user_text:
            return "Draft may be verbose or incomplete; answer with only the required token(s)."
